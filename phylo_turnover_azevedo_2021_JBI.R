...
#all_sq.betapair<-beta.pair(t(end_sq_three_or_more_an), index.family="sor")
...
weights_mat1 <- pd_sd_sq # weight matrix (ex., species richness, PD std)
...
# And here I will run the variable importance function of the gdm R package (gdm.varImp) for all phylogenetic turnover matrices. This may take one entire day (or more) to run with this dataset. If only testing this script, I would recommend to run only 10 permutations if you do not have a powerful computer or if your goal is not use the function with your own data ####
...
# Here I created a function (extract_splines) for getting splines for each variable. Note that lines in all_DF_x is particular to my dataset  #####
...
# Run GDM_DF_function to include the above variables only for each Order and extract the splines
...
# Merging all dataframes with xy spline values ####
...
# Look that this step is particular to the way I named my variables for -grep
